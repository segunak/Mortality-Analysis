...
cdc_data_2005 = pd.read_csv(
    './mortality-data/2005_data.csv', na_values=['NA', '?'])
cdc_data_2006 = pd.read_csv(
    './mortality-data/2006_data.csv', na_values=['NA', '?'])
cdc_data_2007 = pd.read_csv(
    './mortality-data/2007_data.csv', na_values=['NA', '?'])
cdc_data_2008 = pd.read_csv(
    './mortality-data/2008_data.csv', na_values=['NA', '?'])
cdc_data_2009 = pd.read_csv(
    './mortality-data/2009_data.csv', na_values=['NA', '?'])
cdc_data_2010 = pd.read_csv(
    './mortality-data/2010_data.csv', na_values=['NA', '?'])
cdc_data_2011 = pd.read_csv(
    './mortality-data/2011_data.csv', na_values=['NA', '?'])
cdc_data_2012 = pd.read_csv(
    './mortality-data/2012_data.csv', na_values=['NA', '?'])
cdc_data_2013 = pd.read_csv(
    './mortality-data/2013_data.csv', na_values=['NA', '?'])
cdc_data_2014 = pd.read_csv(
    './mortality-data/2014_data.csv', na_values=['NA', '?'])
cdc_data_2015 = pd.read_csv(
    './mortality-data/2015_data.csv', na_values=['NA', '?'])
...
def TopCausesOfDeath(cdc_data, icd_descriptions, n=10):
    ...
    top_n_deaths = deathsByFrequency.head(
        n).rename_axis('Code').reset_index(name='Deaths')

    codeDescriptions = [icd_descriptions[code]
                        for code in top_n_deaths['Code']]
...
icd_desc_2005 = MapIcdToDesc(
    cdc_data_2005["113_cause_recode"], icd_codes_2005["113_cause_recode"])
icd_desc_2006 = MapIcdToDesc(
    cdc_data_2006["113_cause_recode"], icd_codes_2006["113_cause_recode"])
icd_desc_2007 = MapIcdToDesc(
    cdc_data_2007["113_cause_recode"], icd_codes_2007["113_cause_recode"])
icd_desc_2008 = MapIcdToDesc(
    cdc_data_2008["113_cause_recode"], icd_codes_2008["113_cause_recode"])
icd_desc_2009 = MapIcdToDesc(
    cdc_data_2009["113_cause_recode"], icd_codes_2009["113_cause_recode"])
icd_desc_2010 = MapIcdToDesc(
    cdc_data_2010["113_cause_recode"], icd_codes_2010["113_cause_recode"])
icd_desc_2011 = MapIcdToDesc(
    cdc_data_2011["113_cause_recode"], icd_codes_2011["113_cause_recode"])
icd_desc_2012 = MapIcdToDesc(
    cdc_data_2012["113_cause_recode"], icd_codes_2012["113_cause_recode"])
icd_desc_2013 = MapIcdToDesc(
    cdc_data_2013["113_cause_recode"], icd_codes_2013["113_cause_recode"])
icd_desc_2014 = MapIcdToDesc(
    cdc_data_2014["113_cause_recode"], icd_codes_2014["113_cause_recode"])
icd_desc_2015 = MapIcdToDesc(
    cdc_data_2015["113_cause_recode"], icd_codes_2015["113_cause_recode"])

# Finding the top 10 causes of death in the United States for each year.
top_ten_2005 = TopCausesOfDeath(
    cdc_data_2005["113_cause_recode"], icd_desc_2005)
top_ten_2006 = TopCausesOfDeath(
    cdc_data_2006["113_cause_recode"], icd_desc_2006)
top_ten_2007 = TopCausesOfDeath(
    cdc_data_2007["113_cause_recode"], icd_desc_2007)
top_ten_2008 = TopCausesOfDeath(
    cdc_data_2008["113_cause_recode"], icd_desc_2008)
top_ten_2009 = TopCausesOfDeath(
    cdc_data_2009["113_cause_recode"], icd_desc_2009)
top_ten_2010 = TopCausesOfDeath(
    cdc_data_2010["113_cause_recode"], icd_desc_2010)
top_ten_2011 = TopCausesOfDeath(
    cdc_data_2011["113_cause_recode"], icd_desc_2011)
top_ten_2012 = TopCausesOfDeath(
    cdc_data_2012["113_cause_recode"], icd_desc_2012)
top_ten_2013 = TopCausesOfDeath(
    cdc_data_2013["113_cause_recode"], icd_desc_2013)
top_ten_2014 = TopCausesOfDeath(
    cdc_data_2014["113_cause_recode"], icd_desc_2014)
top_ten_2015 = TopCausesOfDeath(
    cdc_data_2015["113_cause_recode"], icd_desc_2015)
...
topTenAllYears['Description'] = [icd_desc_2015[code]
                                 for code in topTenAllYears['Code']]
...
medical_data = pd.read_csv(
    './data-from-canvas/medicaltranscriptions.csv', sep=',', header=0)
pub_med_model = KeyedVectors.load_word2vec_format(
    './data-from-canvas/PubMed-and-PMC-w2v.bin', binary=True)
...
for desc in medical_data['description']:
    ...
    token_desc = [word.lower()
                  for word in token_desc if not word in stop_words]
...
medical_desc_tokenized = [list(set(desc_list))
                          for desc_list in medical_desc_tokenized]
...
for desc in medical_desc_modeled:
    ...
    for code_desc in icd_desc_tokenized:
        ...
        data = (patientId, patient_desc_sentence,
                icd_desc_sentence, cosine_sim)
...
for index, row in final_patient_data.iterrows():
    final_patient_data.loc[index,
                           "Visit Description"] = long_descriptions[row["Patient Id"]]

final_patient_data = final_patient_data.sort_values(
    "Cosine Similarity", ascending=False)
final_patient_data["Cosine Similarity"] = ["{0:.0%}".format(
    sim) for sim in final_patient_data["Cosine Similarity"]]
